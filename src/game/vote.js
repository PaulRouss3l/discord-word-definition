...
var check_vote_submission = async function(client, message) {
    
    let game = Model.get_game(message.channel.id);
    vote = parseInt(message.content, 10);
    ...
    let text = message.author.username + ', ton vote est enregistrée.';
    message.channel.send(text);

    game.has_voted.push(message.author.id);

    if (vote.userId == 1) {// good answer
...
var display_score = function(client, message) {
    console.log('display score');
    let game = Model.get_game(message.channel.id);

    rightAnswer = game.votes.find(voteOption => {
